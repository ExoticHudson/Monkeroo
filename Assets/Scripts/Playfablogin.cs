...
public class Playfablogin : MonoBehaviour
{
    ...
    public void OnLoginSuccess(LoginResult result)
    {
        ...
        // ✅ Added: set display name from PlayerPrefs
        string pUsername = PlayerPrefs.GetString("username");
        if (!string.IsNullOrEmpty(pUsername))
        {
            PlayFabClientAPI.UpdateUserTitleDisplayName(
                new UpdateUserTitleDisplayNameRequest { DisplayName = pUsername },
                r => Debug.Log("Display name set!"),
                e => Debug.Log("Display name error: " + e.GenerateErrorReport())
            );
        }
